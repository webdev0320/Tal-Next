  ...
  <ul className="space-y-2 mt-4">
    {items.map((item, index) => (
      <li key={index} className="flex items-center text-gray-700">
        <span className="text-brand-orange-custom mr-2 text-xl">â—</span> {item}
      </li>
    ))}
  </ul>
);
    ...
    <div className="min-h-screen bg-white">
          ...
          left={
            <div>
              ...
              <p className="text-lg text-gray-700 mb-6">
                What usually happens is slower than that. A property gets rented temporarily after a move. Another property is inherited and let out for â€œjust a few years.â€ Mortgage interest rules change. Records become inconsistent. A return is submitted once, then missed later. Rental income is declared partially but not fully. Expenses are estimated instead of tracked properly. Eventually, years pass. Throughout that time, the landlord often still believes the situation is manageable. Then the HMRC letter arrives. Not aggressive. Not accusatory. Just enough to make the situation suddenly feel real.
              </p>
              <p className="text-xl font-semibold text-brand-teal mb-6">
                That moment is usually when the Let Property Campaign stops feeling like â€œsomething other landlords deal withâ€ and becomes personal.
              </p>
              ...
              {[
                ...
                <div key={card.title} className="border-l-4 border-brand-orange-custom bg-brand-panel-light-grey p-6">
                  <div className="flex justify-between items-center">
                    ...
                    <a href={card.link} className="text-brand-orange-custom font-semibold">Read More Â»</a>
                  </div>
                </div>
              ))}
            </div>
      ...
      <Section>
        <TwoColumnGrid 
          left={
            <div>
              <h2 className="text-3xl font-bold text-brand-teal mb-4">HMRC Let Property Campaign â€“ What It Actually Is</h2>
          ...
          right={<img src="/images/2023/08/young-asian-household-woman-busy-with-bills-and-ta-2023-03-13-21-42-15-utc-scaled.jpg" alt="Let Property Campaign" className="rounded-lg shadow-lg w-full object-cover" />}
        />
      </Section>
      ...
      <Section>
        <h2 className="text-3xl font-bold text-brand-teal mb-4 text-center">Let Property Campaign Disclosure â€“ What HMRC Usually Looks At</h2>
      ...
      <Section>
        <TwoColumnGrid 
          left={<img src="/images/2026/05/Services-Taxation-Let-Property-Campaign-The-Difference-Between-a-Prompted-Unprompted-Disclosure.webp" alt="Let Property Campaign" className="rounded-lg shadow-lg w-full object-cover" />}
          right={
            <div>
      ...
      <Section>
        <div className="border-4 border-brand-orange-custom p-8">
          <h3 className="text-2xl font-bold text-brand-teal mb-4">Insight Section â€“ Most Let Property Campaign Problems Start Long Before Tax Gets Missed</h3>
          <p className="text-lg text-gray-700 mb-4">The largest Let Property Campaign liabilities are rarely created by one dramatic mistake. Usually, they build quietly through assumptions. A landlord assumes short rental periods â€œprobably do not matter.â€ Someone believes overseas income only matters where money enters the UK. Joint ownership percentages are assumed instead of formally documented. Mortgage interest is claimed incorrectly for years because â€œthat is how it was always done.â€ Repairs and improvements become mixed together without proper tracking.</p>
          ...
          <div className="grid grid-cols-2 gap-4 mb-4 text-brand-teal font-semibold">
            {["disclosure timing", "penalty mitigation", "evidence support", "record reconstruction", "payment planning", "strategic correction"].map(item => <div key={item}>â— {item}</div>)}
          </div>
      ...
      <Section>
        <TwoColumnGrid 
          reverse={true}
          left={<img src="/images/2026/05/Services-Taxation-Let-Property-Campaign-Let-Property-Campaign-and-Non-UK-Resident-Landlords.webp" alt="Let Property Campaign" className="rounded-lg shadow-lg w-full object-cover" />}
          right={
            <div>
      ...
      <Section>
        ...
        <div className="grid md:grid-cols-2 gap-4 text-brand-teal font-semibold">
          {["reviewing historic rental income", "rebuilding incomplete records", "assessing ownership structures", "calculating tax, interest, and penalties", "reviewing capital gains exposure", "correcting self assessment returns", "supporting HMRC disclosure correspondence", "analysing overseas property reporting issues"].map(item => <div key={item}>â— {item}</div>)}
        </div>
        <div className="border border-brand-orange-custom p-6 mt-8 font-semibold text-center">The difference is not simply â€œfiling a disclosure.â€ It is creating a position that can still be defended logically if HMRC later reviews how the disclosure was prepared.</div>
      </Section>
      ...
      <Section bg="bg-brand-panel-light-grey">
        ...
        <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
          {reviews.map(rev => <div key={rev.name} className="bg-white p-6 shadow rounded">â­â­â­â­â­<p className="font-bold mt-2">{rev.name}</p></div>)}
        </div>
      </Section>
      ...
      <Section bg="bg-brand-panel-light-grey">
        <TwoColumnGrid 
          reverse={true}
          left={<img src="/images/2023/08/young-asian-household-woman-busy-with-bills-and-ta-2023-03-13-21-42-15-utc-scaled.jpg" alt="Let Property Campaign" className="rounded-lg shadow-lg w-full object-cover" />}
          right={
            <div>
      ...
      <Section className="text-center">
        ...
        <div className="grid md:grid-cols-2 gap-4 mb-8 text-brand-teal font-semibold max-w-2xl mx-auto text-left">
          {["HMRC Let Property Campaign disclosures", "rental income corrections", "self assessment amendments", "capital gains tax reviews", "non-resident landlord tax issues", "disclosure calculations", "penalty mitigation support", "historic property tax reconstruction"].map(item => <div key={item}>â— {item}</div>)}
        </div>
        ...
        <div className="space-y-4">
          {[
            ...
            <div key={i} className="border border-brand-teal">
              <button onClick={() => setOpenFaq(openFaq === i ? null : i)} className="w-full p-4 text-left font-bold bg-brand-teal text-white flex justify-between">{item.q} <span>{openFaq === i ? 'âˆ’' : '+'}</span></button>
